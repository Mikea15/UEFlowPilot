...
Right now, the AI we're spawning is using the Actor where `UFlowPilotComponent` was added, as the source location.
This is done with the Property `Spawn Location Reference`. This property is a custom type called `FlowActorReference`.

You can read more about <LinkRef c="FlowActorReference" />
...
There's a third `Scope` option for `FlowActorReferences` which is `Runtime`.
...
FlowPilot includes a `UTagActorComponent` component to make adding Tags to actors easy. 
</Aside>

## Wrapping Up

You should now have a basic understanding of how to create, edit, and run FlowPilot. 
We covered how to create an asset, linking the asset to a FlowPilotComponent, spawn actors in game, and use GameplayTags to find Actors.
